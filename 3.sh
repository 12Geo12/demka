...
# ИСПРАВЛЕНИЯ:
# - Улучшено определение имени физического интерфейса
# - Поддержка разных naming conventions (eth0, enp7s3, ens33, etc.)
# - Автоматическое определение базового интерфейса для VLAN
# ==============================================================================
...
# ИСПРАВЛЕНИЕ: Определение типа интерфейса
get_iface_type() {
    local iface=$1
    
    # Проверяем, это VLAN (содержит точку)
    if [[ "$iface" == *"."* ]]; then
        echo "VLAN"
        return
    fi
    
    # Проверяем type в /sys/class/net
    if [ -f "/sys/class/net/${iface}/type" ]; then
        local type=$(cat "/sys/class/net/${iface}/type" 2>/dev/null)
        case "$type" in
            1) 
                # Ethernet или другой физический интерфейс
...
# ИСПРАВЛЕНИЕ: Получение списка ФИЗИЧЕСКИХ интерфейсов
get_physical_interfaces() {
    ...
    for iface in $(ls /sys/class/net/ 2>/dev/null); do
        ...
        case "$iface" in
            lo|docker*|veth*|virbr*|sit*|br-*|flannel*|cni*|tun*|tap*|bond*|gre*)
                continue
                ;;
        esac
        
        # Пропускаем VLAN интерфейсы (содержат точку)
        if [[ "$iface" == *"."* ]]; then
            continue
        fi
        
        # Проверяем, это физический интерфейс
        local iface_type=$(get_iface_type "$iface")
        if [ "$iface_type" = "PHYSICAL" ]; then
            phys_ifaces="$phys_ifaces $iface"
        fi
    done
...
# ИСПРАВЛЕНИЕ: Получение списка всех интерфейсов (физические + VLAN)
get_all_interfaces() {
    local all_ifaces=""
...
# Показ интерфейсов (ИСПРАВЛЕНИЕ: улучшен вывод)
show_interfaces() {
    echo ""
    echo -e "${CYAN}========================================================${NC}"
    ...
    # ИСПРАВЛЕНИЕ: Показываем физические интерфейсы отдельно
    local phys_ifaces=$(get_physical_interfaces)
    local vlan_ifaces=$(ls /sys/class/net/ 2>/dev/null | grep -E "\.")
    
    if [ -z "$phys_ifaces" ] && [ -z "$vlan_ifaces" ]; then
    ...
    if [ -n "$phys_ifaces" ]; then
        echo -e "${GREEN}=== Физические интерфейсы ===${NC}"
        echo ""
    ...
    if [ -n "$vlan_ifaces" ]; then
        echo ""
        echo -e "${CYAN}=== Существующие VLAN интерфейсы ===${NC}"
        echo ""
        
        for iface in $vlan_ifaces; do
            # Пропускаем виртуальные интерфейсы
            case "$iface" in
                docker*|veth*|virbr*|br-*)
                    continue
                    ;;
            esac
...
select_interface() {
    local idx=$1
    local phys_ifaces=$(get_physical_interfaces)
    echo "$phys_ifaces" | awk -v n="$idx" '{print $n}'
}
...
echo -e "${CYAN}║${NC}         ${YELLOW}(Improved Interface Detection)${NC}                   ${CYAN}║${NC}"
echo -e "${CYAN}╚══════════════════════════════════════════════════════════╝${NC}"
...
echo "  3) Удалить VLAN"
echo "  4) Показать все интерфейсы"
echo "  5) Выход"
...
PHYS_IFACES=$(get_physical_interfaces)
IFACE_COUNT=$(echo "$PHYS_IFACES" | wc -w)
...
# ИСПРАВЛЕНИЕ: Убедимся, что физический интерфейс имеет корректный конфиг
PHYS_DIR="$IFACES_DIR/${PHYS_IFACE}"
if [ ! -d "$PHYS_DIR" ]; then
    echo -e "${YELLOW}Создание конфигурации для $PHYS_IFACE...${NC}"
    mkdir -p "$PHYS_DIR"
    
    # Определяем тип интерфейса для конфигурации
...
EOF
    ...
    # Запрашиваем IP для физического интерфейса
    echo ""
    read -p "Назначить IP для $PHYS_IFACE? (y/n) [n]: " assign_ip
    if [[ "$assign_ip" =~ ^[Yy] ]]; then
        read -p "Введите IP адрес (например, 192.168.1.1/24): " phys_ip
        if [ -n "$phys_ip" ]; then
            echo "$phys_ip" > "$PHYS_DIR/ipv4address"
            echo -e "${GREEN}IP $phys_ip назначен для $PHYS_IFACE${NC}"
        fi
    fi
fi
