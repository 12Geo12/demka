...
# ============================================================================
# ОПРЕДЕЛЕНИЕ ЛОКАЛЬНОГО IP (ИСПРАВЛЕНО ДЛЯ ALT LINUX)
# ============================================================================
# Используем ip addr вместо hostname -I
...
# ============================================================================
# УСТАНОВКА ПАКЕТОВ (ИСПРАВЛЕНО ДЛЯ ALT LINUX)
# ============================================================================
log_step "Установка пакетов..."

# Проверяем доступный пакетный менеджер
if command -v apt-get &> /dev/null; then
    PKG_MGR="apt-get"
    log_info "Используется apt-get (ALT Linux)"
    apt-get update >/dev/null 2>&1 || true
elif command -v yum &> /dev/null; then
    PKG_MGR="yum"
    log_info "Используется yum"
elif command -v dnf &> /dev/null; then
    PKG_MGR="dnf"
    log_info "Используется dnf"
else
...
$PKG_MGR install -y bind bind-utils firewalld
...
# Определяем имя службы (в ALT Linux обычно named)
