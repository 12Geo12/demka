...
# Версия: 3.3 - ИСПРАВЛЕНО для ALT Linux
#===============================================================================
set -e
...
check_persistence() {
    ...
    if [[ -f /etc/frr/vtysh.conf ]]; then
        ...
        echo -e " ${RED}✗${NC} vtysh.conf не найден"
    fi
}
...
apply_configuration() {
    ...
    sleep 2
    if ip link show gre1 &>/dev/null; then
        log_ok "Туннель активен"
    else
        log_err "Туннель не активен!"
        return 1
    fi
    ...
    if [[ ! -f "$FRR_DAEMONS" ]]; then
        ...
        if [[ ! -d "/etc/frr" ]]; then
            mkdir -p /etc/frr
            log_info "Создана директория /etc/frr"
        fi
...
EOF
    ...
    # Проверяем существует ли служба frr
    if ! systemctl list-unit-files | grep -q frr; then
        log_err "FRR не установлен!"
        log_info "Установите: apt-get install frr frr-pythontools"
        return 1
    fi
    
    if systemctl restart frr; then
        sleep 5  # Увеличенная пауза для ALT Linux
        if systemctl is-active frr &>/dev/null; then
...
while true; do
    clear
    echo -e "${CYAN}========================================${NC}"
    echo -e "${CYAN} Настройка GRE туннеля и OSPF v3.3${NC}"
    echo -e "${CYAN}========================================${NC}"
    echo ""
